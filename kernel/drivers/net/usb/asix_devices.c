...
static u8 CMDLINEMAC[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
static u8 DEFMAC[] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
static u8 DEFEEPROMMAC[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x01};
...
static int ax88772_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (ret < 0) {
        ...
        for(i=0;i < ETH_ALEN;i++){
                if (DEFEEPROMMAC[i] != buf[i]){
                        ret = -1;
                        printk(KERN_ERR " user eeprom mac diff-%d 0x%x != 0x%x \n",i ,DEFEEPROMMAC[i], buf[i] );
                        break;
                }
        }
        ...
        if (!ret){
                for(i=0;i < ETH_ALEN;i++){
                        if (CMDLINEMAC[i] != DEFMAC[i]){
                                memcpy(buf, CMDLINEMAC, 6);
                                printk(KERN_ERR "use cmdline mac  diffi-%d 0x%x != 0x%x \n",i ,CMDLINEMAC[i], DEFMAC[i] );
                                break;
                        }
                }
        }
...
static int __init get_mac_addr(char *line)
{
        unsigned char mac[6];
        int i = 0;
        ...
        memcpy(CMDLINEMAC, mac, 6);
        printk(KERN_ERR "boot setup mac-addr: %x:%x:%x:%x:%x:%x\n",
                CMDLINEMAC[0], CMDLINEMAC[1], CMDLINEMAC[2], CMDLINEMAC[3], CMDLINEMAC[4],
                CMDLINEMAC[5]);
        return 1;
}

__setup("r8152.mac=", get_mac_addr);
